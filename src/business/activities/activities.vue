       ...
       <flexbox :gutter="0">
          ...
          <flexbox-item :span="1/4" class="ct">
              <div class="filter-rule" :class="{
                 on : showDstDropdown ,
                 active : filterData.trading_area_id.length || filterData.district_id.length || filterData.subway_station_id.length }"
...
export default {
  ...
  components: { Flexbox , FlexboxItem , vActivitylist , LoadMore ,AppDown , vInfo },
  mixins : [ loadMore ] ,
  ...
  async mounted(){
    ...
    if( JSON.stringify( actChildData ) == "{}"|| JSON.stringify( actChildData ) == "null" ){
      ...
      if(   this.filterData.facilities
        ...
        ||this.filterData.activity_type_id
    ...
    if( !config ){
      ...
      this.fieldSort = configsync.result.field_sort ;
    ...
    defaultInit(){
      ...
      getResources( params ).then( res => {
        ...
        wxshare({
          ...
          link : window.location.origin + '?from=singlemessage'+ window.location.hash + '?is_app=1' ,
        });
      }).catch(err => {
        // 隐藏小的loading
        this.is_show = false ;
