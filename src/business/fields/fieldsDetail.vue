  ...
  <div class="detail-wraper">
    ...
    <section class="resource" v-if="fieldsdetails">
      ...
      <div class="address">
        ...
        <div class="adress-icon"  @click.stop="goMap(fieldsdetails.community.lng ,fieldsdetails.community.lat , fieldsdetails.name)">
          ...
          <div class="item" v-for="(item,index) in enquiry_resource" :key="index">
            <div class="enquire-flex">
              ...
              <div class="enquire-btns">
                ...
                <router-link v-if="item.identification" :to="{ name : 'enquire', params : { id : item.id } }" class="enquire-btn-self"></router-link>
                <router-link v-else :to="{ name : 'enquire', params : { id : item.id } }" class="enquire-btn" :class="{ disabled : item.expired }">询价</router-link>
    ...
    <section class="res-detail" ref="resDetail">
      ...
      <div class="cont" :class="{ 'viewall' : isViewAll }" v-if="fieldsdetails">
          <div class="com-info">
            ...
            <div class="com-info-item" v-if="fieldsdetails.community.total_floor || fieldsdetails.community.total_floor == 0">
              <span class="com-info-tit">总楼层：</span><span class="com-info-cnt">{{ fieldsdetails.community.total_floor }}</span>
            </div>
          </div>
    ...
    <section class="other-res" v-if="otherResource.length">
      <div class="tit">
        ...
        <router-link to="/fields" class="view-more">
          <span>查看全部</span><span class="right-icon"></span>
        </router-link>
      </div>
      <div class="cont">

          <router-link class="item" :to="{ name : 'fieldsDetail' , params : { id : item.id } , query : { res_type_id : item.res_type_id } }" v-for="(item,index) in otherResource" :key="index">
            <div class="img-box">
    ...
    <section class="other-res" v-if="list.length">
      ...
      <div class="cont">
        <router-link class="item" :to="{ name : 'fieldsDetail' , params : { id : item.resource_id } , query : { res_type_id : item.res_type_id } }" v-for="(item,index) in list" :key="index">
          <div class="img-box">
            <img v-lazy="item.pic_url" :data-srcset="item.pic_url | half" alt=" ">
            <div class="label-icon" v-if="item.res_type_id==3"></div>
            <!--活动  限时-->
    ...
    <!--魔窗-->
    <app-down v-show="fieldsdetails" :params="{ resource_id : $route.params.id , type_id : res_type_id }" link="https://aj4j3o.mlinks.cc/Adoh"></app-down>
    <!--子路由-->
...
//      this.$vux.loading.hide();
  ...
  methods : {
    ...
    addCart( num ){
      ...
      this.$store.dispatch('setProductDetailsToCar', arr);
      let login = this.$store.state.login || getSession('login');
      if(!login){
        this.$router.push( { name :'login' , query : { redirect : this.$route.fullPath } } );
        return false ;
      ...
      if( this.cancleData.lease_term_type_name === '天' ){
        for(let item of this.dateArr){
          let obj = {
            ...
            name : this.fieldsdetails.name , // 场地名
