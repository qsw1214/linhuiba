<template lang="html">
  <div>
    <v-head isFixed="true" path="/myInvoice">
    ...
    <div class="fee-detail">
      ...
      <div class="item" v-if="is_paper!=2">
        <div class="item-lf">
          <span><span class="info-icon" @click.stop="showWindow=true"></span>快递费：</span>
...
<script>
  ...
  import { getDefaultAddress , postApplyInvoice ,bulidInvoiceOrder , getInvoiceInfo , getInvoiceTitle , getInvoiceRate } from 'src/service/getData';
...
export default {
  ...
  created(){
    ...
    this.total_fee = +this.$route.query.sum || 0 ;
  ...
  methods : {
    ...
    ticketClick(item){
      if(!item.disabled){
        this.$router.replace( { name : 'applyInvoice' , params : { id : item.id } , query : this.$route.query } );
      }else{
    ...
    payBtn(){
      ...
      let params = {
        ...
        order_item_ids : JSON.stringify(this.order_items)
      };
      if( this.ticket_remarks ){
        params.ticket_remarks = this.ticket_remarks ;
      }
      ...
      if (+this.is_paper === 2) {
        ...
        params.freight_collect = +this.freight_collect ;
        if( +this.freight_collect === 0 ){
    ...
    amontTotalFee(){
      if( this.ticket_type != 1 ){
        this.total_tax = this.total_fee*this.rate/10000 ;
      }
    }
  },
  async mounted(){
