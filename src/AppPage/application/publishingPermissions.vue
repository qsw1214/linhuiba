<template>
  <div class="publishing-permissions">
    ...
    <div class="clear other-login">
        <div class="right">
          <router-link class="phone-login" :to="{ path : '/appAddAccount' }">添加更多账号<span class="arrow"></span></router-link>
        </div>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      getUserPermissionData () {
        ...
        if (this.status == -1) {
          this.phone = this.$route.query.publishingMobile
        }
      },
      getUserInfor () {
      ...
      applyForBublishSuccess() {
         setupWebViewJavascriptBridge(function(bridge) {
          bridge.callHandler('applyForBublishSuccess', '保存成功' , function (data) {}) ;
        });
      }
    }
  }
