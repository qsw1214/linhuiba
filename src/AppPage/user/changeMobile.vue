<template lang="html">
  <div class="change-mobile-wrap">
      <section class="CM-content">
        ...
        <div class="CM-bottom">
          <input class="CM-input" type="number" disabled v-model='oldMobile'  placeholder="请输入手机号码" />
        </div>
        <div class="CM-center">
          <input class="bor-b" placeholder="请输入短信验证码" maxlength="6" type="text" v-model='data.old_mobile_captcha' @input="inputFunc"/>
          <span  :class="oldMobile > 0 ? 'send' : 'noSend'" @click.stop="sendOldCaptcha" ref="oldCaptcha">{{oldInterval?`已发送(${oldInterval})`:'发送验证码'}}</span>
        </div>
      ...
      <div class="right other-way" v-if="enterprise_role > 0">
        <router-link class="phone-login" :to="{ path : '/appLicenseToChangePassworld' }">若无法获取原手机验证码，请点击这里<span class="rigt-arrow"></span></router-link>
      </div>
...
export default {
  ...
  mounted() {
    if (this.$route.query.oldMobile) {
      this.oldMobile = this.$route.query.oldMobile
    }
    this.enterprise_role = this.$route.query.enterprise_role
  ...
  methods: {
    ...
    checkNewMobile() {
      ...
      setUserMobile(this.data).then(res => {
        ...
        this.appSuccessCallback(this.data.new_mobile)
      }).catch(err => {
        // 隐藏
        this.$vux.toast.text(`新手机号码${err.msg}`, 'middle');
        this.$vux.loading.hide()
    ...
    appSuccessCallback (newMobile) {
      setupWebViewJavascriptBridge(function(bridge) {
        bridge.callHandler('modifyMobileSuccess', newMobile, function (data) {}) ;
      })
    }
  }
