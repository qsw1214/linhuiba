  ...
  <div class="register-wraper">
    ...
    <div v-show="showTap === 1">
      ...
      <div class="upload-comment">
        ...
        <p>2. 请确认营业执照完整性，信息清晰可见，内容真实有效；如遇问题，可咨询客服热线<span @click="telUs('4001028028')">400-1028-028</span>(09:00-18:00)。</p>
      </div>
    ...
    <div v-show="showTap === 4">
      <form action="javascript:;" class="register-model">
        <label class="register-com border-1px">
          <!-- <span class="add-pre">+86</span> -->
          <input type="text" placeholder="请输入账号" autocomplete="off" class="mobile" v-model.trim="companyAccount" @input="inputFunc" >
        </label>
        <label class="register-com border-1px p-51">
          <!-- <span class="com-left">密码</span> -->
          <input
...
import { sendMsg , captchaBoolean , register, registeredCompanyAccount, licenceIdentifyRegistered, userCertified } from 'src/service/getData' ;
// import { licenceIdentify } from 'src/service/getData'
  ...
  methods : {
          ...
          .then( res => {
            ...
            register( params )
              .then( res => {
                ...
                // 返回首页
                this.appPersonSuccess(res.result)
              })
              .catch( err => {
                this.$vux.toast.show({
                  text : err.msg ,
    ...
    postCompanyRegister () {
      if( this.is_agree ){
        ...
        if (!COMPANY_ACCOUNT.test(this.companyAccount)) {
          this.$vux.toast.text('请输入用户名，限字母、数字和下划线', 'middle') ;
          return;
        }
          ...
          .then( res => {
            ...
            userCertified(username).then(res => {
              ...
              .then( res => {
                ...
                // 返回首页
                this.appCompanySuccess(res.result)
              })
              .catch( err => {
                this.$vux.toast.show({
                  text : err.msg ,
    ...
    appCompanySuccess (companyAccount) {
      setupWebViewJavascriptBridge(function(bridge) {
        bridge.callHandler('registerSuccess', companyAccount , function (data) {}) ;
      });
    },
    appPersonSuccess (account) {
      setupWebViewJavascriptBridge(function(bridge) {
        bridge.callHandler('registerSuccess', account , function (data) {}) ;
      });
    },
    telUs(tel){
      setupWebViewJavascriptBridge(function(bridge) {
